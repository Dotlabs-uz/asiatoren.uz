...
export default function FestivalsForm({
    initialData,
    onSubmit,
    onCancel,
}: {
        ...
        try {
            ...
            if (previewFile) {
                pUrl = await uploadFile(
                    previewFile,
                    `festivals/previews/${Date.now()}_${previewFile.name}`
                );
            }
            ...
            if (galleryFiles.length > 0) {
                const uploads = galleryFiles.map((f) =>
                    uploadFile(f, `festivals/gallery/${Date.now()}_${f.name}`)
                );
