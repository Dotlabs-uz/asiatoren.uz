...
import { MultilingualText, News, NewsFormData } from "@/types";
...
export default function NewsForm({
    ...
    const [formData, setFormData] = useState<NewsFormData>({
        ...
        content: initialData?.content || { ru: "", en: "", uz: "" },
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            let pUrl = formData.previewImageUrl;
            if (previewFile) {
                pUrl = await uploadFile(
                    previewFile,
                    `news/previews/${Date.now()}_${previewFile.name}`
                );
            }

            let gUrls = [...formData.images];
            if (galleryFiles.length > 0) {
                const uploads = galleryFiles.map((f) =>
                    uploadFile(f, `news/gallery/${Date.now()}_${f.name}`)
                );
    ...
    return (
        ...
        >
            ...
            <div className="space-y-4">
                <Label className="text-base font-semibold">Текст новости</Label>
